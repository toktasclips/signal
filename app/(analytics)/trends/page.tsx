...
interface MomentumRow {
  label: string
  key: keyof typeof mockMetrics[0]
  prefix: string
  suffix: string
  decimals: number
}
...
export default function TrendsPage() {
  const sorted = [...mockMetrics].sort((a, b) =>
  ...
  const getVals = (key: keyof typeof mockMetrics[0]) =>
    sorted.map((m) => Number(m[key] ?? 0))

  const trendCards = [
    {
      label: "Toplam Gelir",
      key: "cash_collected" as keyof typeof mockMetrics[0],
      prefix: "₺",
      suffix: "",
    },
    {
      label: "Kâr",
      key: "profit" as keyof typeof mockMetrics[0],
      prefix: "₺",
      suffix: "",
    },
    {
      label: "ROAS",
      key: "roas" as keyof typeof mockMetrics[0],
      prefix: "",
      suffix: "x",
    },
    {
      label: "Instagram Reach",
      key: "instagram_reach" as keyof typeof mockMetrics[0],
      prefix: "",
      suffix: "",
    },
    {
      label: "Yeni Müşteri",
      key: "new_customers" as keyof typeof mockMetrics[0],
      prefix: "",
      suffix: "",
    },
    {
      label: "Watch Hours",
      key: "youtube_watch_hours" as keyof typeof mockMetrics[0],
      prefix: "",
      suffix: " sa",
    },
  ]
    ...
    <main className="flex-1 overflow-y-auto bg-[#0A0A0A]">
      ...
      <div className="sticky top-0 z-10 bg-[#0A0A0A]/90 backdrop-blur-sm border-b border-[#1A1A1A] px-6 py-4 lg:pl-6 pl-14">
        ...
        <p className="text-xs text-zinc-600 mt-0.5">
          2024 yıllık trend ve otomatik analiz
        </p>
      </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {trendCards.map((card) => {
              ...
              const sparkline = allVals.slice(-6)
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#1A1A1A]">
                    <th className="text-left text-xs text-zinc-600 uppercase tracking-wider px-5 py-3 font-medium">
                      Metrik
                    </th>
                    <th className="text-right text-xs text-zinc-600 uppercase tracking-wider px-4 py-3 font-medium">
                      Bu Ay
                    </th>
                    <th className="text-right text-xs text-zinc-600 uppercase tracking-wider px-4 py-3 font-medium">
                      Geçen Ay
                    ...
                    <th className="text-center text-xs text-zinc-600 uppercase tracking-wider px-4 py-3 font-medium">
                      Trend
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {momentumRows.map((row, idx) => {
                    ...
                    const change = calcChange(current, prevVal)
                    const allVals = getVals(row.key)
                    const trend = calcTrend(allVals)
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-[#161616] last:border-0 ${
                          idx % 2 === 0 ? "bg-transparent" : "bg-[#0D0D0D]/40"
                        }`}
                      >
                        <td className="px-5 py-3 text-zinc-300 font-medium">
                          {row.label}
                        </td>
                        <td className="px-4 py-3 text-right text-zinc-100 tabular-nums font-medium">
                          {formatVal(current, row.prefix, row.suffix, row.decimals)}
                        </td>
                        <td className="px-4 py-3 text-right text-zinc-500 tabular-nums">
                          {formatVal(prevVal, row.prefix, row.suffix, row.decimals)}
                        </td>
                        <td
                        ...
                        <td className="px-4 py-3 text-center">
                          <TrendIcon
                            size={15}
                            className={`inline-block ${trendColor}`}
                          />
                        </td>
                      </tr>
                    )
